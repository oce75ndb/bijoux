<nav x-data="{ open: false, userDropdown: false, guestDropdown: false, categoryDropdown: false }">
    ...
    <header class="bg-gold dark:bg-brown shadow-md py-6 animate-fade-in-down">
        <div class="container mx-auto px-4 flex justify-between items-center">
            ...
            <a href="{{ route('home') }}"
                class="flex items-center space-x-3 text-brown dark:text-beige hover:text-beige transition-all duration-300">
                <!-- Logo -->
            ...
            <button @click="open = !open"
                class="md:hidden text-brown dark:text-beige focus:outline-none transition-transform transform hover:scale-110">
                ☰
            </button>
            ...
            <nav class="hidden md:flex space-x-6 items-center">
                ...
                <form action="{{ route('produits.index') }}" method="GET" class="relative w-full md:w-80">
                    ...
                    <button type="submit"
                        class="absolute right-3 top-1/2 transform -translate-y-1/2 text-brown dark:text-beige transition-transform transform hover:scale-110">
                ...
                <a href="{{ route('panier.index') }}"
                    class="font-bold hover:text-beige text-brown dark:text-beige transition-all duration-300 hover:scale-105">
                    Panier
                ...
                @guest
                    ...
                    <div class="relative">
                        ...
                        <ul x-show="guestDropdown" @click.away="guestDropdown = false"
                            ...
                            x-transition:enter="opacity-1 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                            x-transition:leave="opacity-100 translate-y-0" x-transition:leave-end="opacity-1 translate-y-2">
                            <li><a href="{{ route('login') }}"
                                    class="block px-4 py-2 hover:bg-gold dark:hover:bg-gray-700">Connexion</a></li>
                            <li><a href="{{ route('register') }}"
                                    class="block px-4 py-2 hover:bg-gold dark:hover:bg-gray-700">Inscription</a></li>
                        </ul>
                    </div>
                @endguest
                        ...
                        <ul x-show="userDropdown" @click.away="userDropdown = false"
                            ...
                            <li>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="block px-4 py-2 w-full text-left hover:bg-gold dark:hover:bg-gray-700">
                                        Déconnexion
                                    </button>
                                </form>
                            </li>
