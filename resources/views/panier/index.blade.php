<x-app-layout :categories="$categories">
    <div class="container mx-auto px-4 py-8">
        ...
        <!-- Titre principal -->
        <h1 class="text-3xl font-bold text-brown dark:text-beige mb-6 text-center animate-fade-in">
            Votre Panier
        </h1>
        ...
        @if(count($panier) > 0)
                            ...
                            <tr class="animate-slide-up delay-100ms]">
                                ...
                                <td class="border border-brown px-4 py-3 flex items-center space-x-4">
                                    @if ($article['image'])
                                        <img src="{{ asset($article['image']) }}"
                                             alt="{{ $article['nom'] }}"
                                             class="w-16 h-16 rounded shadow-md transition-transform transform hover:scale-105">
                                    @endif
                                ...
                                <td class="border border-brown px-4 py-3 text-center">
                                    <div class="flex items-center justify-center space-x-3">
                                        ...
                                        <form method="POST" action="{{ route('panier.decrementer', $id) }}">
                                            ...
                                            @method('PATCH')
                                            <button type="submit" 
                                                    class="bg-gray-200 dark:bg-brown text-brown px-3 py-1 rounded-full hover:bg-gray-300 transition-transform transform hover:scale-105 active:scale-95"
                                ...
                                <td class="border border-brown px-4 py-3 text-center">
                                    <form method="POST" action="{{ route('panier.supprimer', $id) }}">
                                        ...
                                        @method('DELETE')
                                        <button type="submit" 
                                                class="font-bold text-red-500 hover:text-red-700 transition-transform transform hover:scale-110">
                                            ❌ Supprimer
                                        </button>
                                    </form>
                                </td>
            ...
            <div class="mt-6 text-center animate-slide-up delay-200">
                <a href="{{ route('checkout') }}" 
                   class="bg-gold text-white dark:bg-brown dark:text-beige px-6 py-3 rounded-lg text-lg font-semibold shadow-md hover:bg-brown dark:hover:bg-gold transition-transform transform hover:scale-105 active:scale-95">
                    Passer à la caisse
                </a>            
            </div>
        ...
        @else
            ...
            <div class="text-center mt-10 animate-fade-in">
                ...
                <a href="{{ route('produits.index') }}" 
                   class="mt-4 inline-block bg-gold text-white dark:bg-brown dark:text-beige px-6 py-3 rounded-lg shadow-md hover:bg-brown dark:hover:bg-gold transition-transform transform hover:scale-105 active:scale-95">
